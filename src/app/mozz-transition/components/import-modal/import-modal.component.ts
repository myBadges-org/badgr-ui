...
export class ImportModalComponent extends BaseDialog implements OnInit {
		...
		Object.keys(manifest).forEach((email) => {
			manifest[email].forEach((f, i) => {
				...
				reader.onload = (e) =>
					setTimeout(
						() =>
							this.uploadImage(email, reader.result).then(() => {
								this.inProgress--;
								// Are we done with only successes?
								if (!this.inProgress && this.successes && !this.serverErrors && !this.unverifiedEmails)
									this.displaySuccess();
							}),
						i * delayInMS
					);
		...
		return (this.badgeUploadPromise = this.recipientBadgeManager
			...
			.catch((err) => {
				...
				if (!message.match(/^[A-Za-z]/)) {
					if (!this.serverErrors.includes(this.unknownServerError))
						this.serverErrors.push(this.unknownServerError);
					return false;
				}
				...
				switch (message) {
					...
					case 'You already have this badge in your backpack':
						if (!this.serverErrors.includes(this.duplicateError))
							this.serverErrors.push(this.duplicateError);
						break;
	...
	fileChanged(event) {
		this.file =
			event.target.files && event.target.files.length ? event.target.files[0] : event.dataTransfer.files[0];
		...
		Promise.all(
			...
			if (successes)
				this.messageService.reportMajorSuccess(
					`${successes} email ${successes > 1 ? 'addresses' : 'address'} will be verified.`
				);
			if (errors)
				this.messageService.reportAndThrowError(
					`${errors} email ${errors > 1 ? 'addresses' : 'address'} can not be verified.`
				);
			reupBadges.forEach((email) =>
				email.base64File.forEach((base64File) => this.uploadImage(email.email, base64File))
			);
		});
	}
