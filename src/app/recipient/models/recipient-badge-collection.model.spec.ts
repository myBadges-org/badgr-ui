...
describe('RecipientBadgeCollection', () => {
	...
	it('should be constructable', inject([CommonEntityManager], (commonManager: CommonEntityManager) => {
		new RecipientBadgeCollection(commonManager);
	}));

	it('should correctly alias fields', inject([CommonEntityManager], (commonManager: CommonEntityManager) => {
...
export function verifyRecipientBadgeCollection(
	...
	expect(collection.badgeEntries.entities.map((e) => e.badgeSlug)).toEqual(
		apiCollection.badges.map((e) => String(e.id))
	);
	expect(collection.badgeEntries.entities.map((e) => e.description)).toEqual(
		apiCollection.badges.map((e) => e.description)
	);
}
